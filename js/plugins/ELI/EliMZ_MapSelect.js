...
@plugindesc ♦5.0.2♦ Creates a scene that you can select a map to go.
...
@type struct<buttonUiST>
@desc Setup the on screen buttons configurations for the Map select scene.
@default {"cancel":"true","page":"true","position":"top"}
@parent separator3

@command addNewMap
@text New Map Commands
@desc Add a new map to the map select window.
...
@command callScene
@text Open Map Select Scene
@desc Open Map Select Scene
...
@text Enable cancel
@type boolean
@desc Choose to enable or disable cancel button.
@default true

@param page
@text Enable arrows
@type boolean
@desc Choose to enable or disable the arrow buttons.
@default true
...
/* ---------------------------- SPRITE MAP BUTTON --------------------------- */
class Sprite_SelectMapButton extends Sprite {
    ...
    isVisible(){
        return !$gameSwitches.disableMapSelect() && !$gameMessage.isBusy() && ConfigManager.touchUI && !$gameMap.isEventRunning()
    }
...
class Scene_MapSelect extends Scene_MenuBase{
    ...
    helpAreaHeight() {
        return this.calcWindowHeight(Plugin.param().helpLines, false)
    }

/* ------------------------------ BUTTON CONFIG ----------------------------- */

    needsCancelButton(){
        return Plugin.param().sceneButton.cancel
    }

    needsPageButtons(){
        return Plugin.param().sceneButton.page
    }
    ...
    createPageButtons(){
        ...
        this._pageupButton.setClickHandler(this.pageUpIndex.bind(this))
        this._pagedownButton.setClickHandler(this.pageDownIndex.bind(this))
    }

    pageUpIndex(){
        const command = this._commandWindow

        if(command.index() === 0){
            command.select(command._list.length - 1)
        }else{
            command.cursorUp()
        }
    ...
    pageDownIndex(){
        ...
        if(command.index() === command._list.length - 1){
            command.select(0)
        }else{
            command.cursorDown()
        }
    ...
    isBottomButtonMode(){
        return Plugin.param().sceneButton.position === "bottom"
    }
...
/* ---------------------------- BUTTON CONFIG END --------------------------- */
    ...
    createCommandWindow(){
        ...
        const commandWindow = new Window_MapSelectCommand(rect)
    ...
    autoMainCommandWidth(){
        const dummyWindow = new Window_Command(new Rectangle(0, 0, 200, 200))
        const allCommands = Plugin.getCommands()
        const allNamesWidth = allCommands.map(item => Eli.Utils.getTextWidth(item.name))
        const largestName = Math.floor(Math.max(...allNamesWidth))

        return largestName + dummyWindow.padding * 2 + dummyWindow.itemPadding() * 2 + dummyWindow.colSpacing()
    }
    ...
    createMapWindow(){
        ...
        this._mapWindow = new Window_Preview(rect)
...
Eli.MapSelect = {
    ...
    initPluginCommands(){
        const commands = ['addNewMap', 'callScene']
